...
void* display_string_in_battle(const u8*);
u32 read_word(void*);
u16 rng();
...
void* str_append(const u8* dst, const u8* src);
...
u8 poke_add_to_pc(struct pokemon* poke);
...
void b_animate_opponent_poke(struct object*, u16 species, bool no_cry, u8 bitfield);
void b_animate_player_poke(struct object*, u16 species);
...
void battle_anim_refresh_graphics_maybe(u8 bank, u8 arg1, u8 objID);
u8 task_add(void* function_ptr, u8 priority);
u8 task_is_running(u8 taskID);
void sub_80A4980(u8 taskID);
u32 task_get_priv_u32(u8 taskID, u8 privateID);
void task_set_priv_u32(u8 taskID, u8 privateID, u32 value);
void task_do_poke_animation(u8 taskID);
void task_animate_poke_after_delay(u8 taskID);
u8 task_find_id_by_funcptr(void* function_ptr);
u8 sav1_get_map_battleground(void);
void tilemap_decompress_wram_BGconfig(u8 bgID, void* tilemap, u16 arg2, u16 arg3);
void bgid_send_tilemap(u8 bgID);
void frame_battletextbox(void);
void turn_values_cleanup(u8 arg0);
...
void task_add_01_battle_start(u8 screenchange, u16 songID);
bool is_ov_to_battle_transition_done(void);
void battlestructs_setup(void);
void pokemon_purge_opponent(void);
void sp28_party_to_save(void);
void sp29_party_from_save(void);
