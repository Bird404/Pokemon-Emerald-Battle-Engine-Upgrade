...
void species_change(u8 bank, u16 new_species)
{
...
bool exec_cherrim_script(u8 bank, bool use_end3)
{
    ...
    if (use_end3)
        execute_battle_script(BS_CHERRIMSWITCH_END3);
    else
    {
        battlescript_push();
        battlescripts_curr_instruction = BS_CHERRIMSWITCH;
    }
    return 1;
}
...
bool exec_castform_script(u8 bank, bool use_end3)
{
    ...
    battle_scripting.active_bank = bank;
    if (use_end3)
        execute_battle_script(BS_CASTFORMSWITCH_END3);
    else
    {
        battlescript_push();
        battlescripts_curr_instruction = BS_CASTFORMSWITCH;
    }
    return 1;
}
...
u8 ability_battle_effects(u8 switch_id, u8 bank, u8 ability_to_check, u8 special_cases_argument, u16 move)
{
    ...
    case 0: //switch-in abilities
        ...
        case ABILITY_CLOUD_NINE:
            effect = 1;
            bank_attacker = bank;
            execute_battle_script(BS_AIRLOCK);
            break;
            ...
            case ABILITY_HEALER:
                {
                    ...
                    if (is_bank_present(ally) && battle_participants[ally].status.int_status && percent_chance(30))
                    {
                        ...
                        execute_battle_script(BS_HEALER);
    ...
    case 8: //attacker's synchronize after static etc.
        {
            ...
            if (check_ability(bank, ABILITY_SYNCHRONIZE) && Synchro->synchro_effect[bank])
            {
                ...
                battle_scripting.active_bank = bank;
                battlescript_push();
                battlescripts_curr_instruction = BS_SYNCHRONIZE;
                hitmarker |= 0x2000;
            }
        }
        break;
