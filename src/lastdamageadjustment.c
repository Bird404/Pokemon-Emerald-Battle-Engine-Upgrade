...
void final_damagecalc_cmd7()
{
    ...
    if (!(battle_participants[bank_target].status2 & 0x1000000))
    {
        ...
        else if (battle_participants[bank_target].ability_id == ABILITY_STURDY && has_ability_effect(bank_attacker, 1, 1) && damage_loc > target_hp && target_hp == battle_participants[bank_target].max_hp)
        {
            damage_loc = target_hp - 1;
